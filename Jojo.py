...
class byTaha(Plugin):
    ...
    def S(s,b,a,f,j):
        p = AR.parse(t=a,s=f)
        teck(b,Call(bs.chatmessage,p))
        push(f"{['برابر','شامل'][j]}!\nپاسخ به: {f}\nبا متن: {p}\nبعد از {b} ثانیه!",color=(0,0.8,0.8)) if var('tune0') else None
        gs('dingSmallHigh').play() if var('tune1') else None


# =========================
# Auto Reply (Contains Mode)
# =========================
def check_auto_reply(message: str):
    """
    Auto reply that triggers if keyword appears anywhere in the message.
    Example:
        trigger = 'cu'  -> will match: cu, %cu, 125cu, abcudef
    """
    try:
        triggers = var('l') or {}
        msg = message.lower()
